...
## [1.6.1] — 2026-07 — PDF export: fix a severe slowdown from the longer report
### Fixed
- The new, much longer report (7 sections, ~18 capture blocks) exposed a latent performance issue in PDF export: `html2canvas` re-clones the *entire* live page on every single block capture to preserve accurate computed styles, so its cost scales with total page size. With the report now roughly twice as long, each block's clone could take 50+ seconds — turning what should be a few seconds into potentially 15-20+ minutes, effectively hanging the "Building your PDF…" state well past the existing 20s-per-block/60s-overall safety nets (those timeouts rely on `setTimeout`, which can't fire while the main thread is synchronously busy cloning). Fixed by temporarily hiding every *other* top-level report block (and the floating music button) while each block is captured, shrinking what `html2canvas` has to clone on every call. Verified end-to-end: full 18-block report now exports in ~35 seconds with zero skipped sections, versus 50+ seconds for the *first block alone* before this fix.
